...
class Member_model extends CI_Model {
    ...
    public function get_sns($uid) {
        ...
        return array(
            ...
            'weibo' => $this->db->where('uid', $uid)->count_all_results('sns_feed'),
    ...
    public function get_member($uid) {
        ...
        if (!$data['groupid']
            ...
            if ($group[$data['groupid']]['unit'] == 1
                ...
                // 转为过期的后的会员组
                $data['groupid'] = $data['group']['overdue'] ? $data['group']['overdue'] : 3;
                $this->db
                     ->where('uid', $uid)
                     ->update('member', array(
                        'levelid' => 0,
    ...
    public function admin_login($username, $password) {
        ...
        if (!$this->is_admin_auth($data['adminid'])) {
            return -4;
        } // 站点权限判断
    ...
    public function get_admin_member($uid, $verify = 0) {
        ...
        $data = $this->db // 查询用户信息
                     ->select('m.uid,m.email,m.username,m.adminid,m.groupid,a.realname,a.usermenu')
                     ->from($this->db->dbprefix('member').' AS m')
    ...
    public function get_admin_all($roleid = 0, $keyword = NULL) {
        ...
        if ($roleid) {
            $select->where('b.adminid', $roleid);
        }
        if ($keyword) {
            $select->like('b.username', $keyword);
        }
    ...
    public function OAuth_login($appid, $data) {
        ...
        foreach ($MEMBER['synurl'] as $url) {
            $synlogin.= '<script type="text/javascript" src="'.$url.'/index.php?c=api&m=synlogin&expire='.$expire.'&code='.$this->encrypt->encode($member['uid'].'-'.$member['salt']).'"></script>';
        }
    ...
    public function login($username, $password, $expire, $back = 0) {
        ...
        // 查询会员信息
        $data = $this->db
                     ->select('`uid`, `password`, `salt`, `email`, `username`')
                     ->where('username', $username)
                     ->limit(1)
                     ->get('member')
                     ->row_array();
    ...
    private function _login_log($uid, $OAuth = '', $is_admin = 0) {
        ...
        $agent = ($this->agent->is_mobile() ? $this->agent->mobile() : $this->agent->platform())
            .'&nbsp;&nbsp;&nbsp;&nbsp;'.$this->agent->browser().' v'.$this->agent->version();
    ...
    private function _register($data, $OAuth = NULL, $groupid = NULL, $uid = NULL) {
        ...
        if ($OAuth) {
            ...
            $username = $regoauth && $data['nickname'] ? ($this->db->where('username', $data['nickname'])->count_all_results('member') ? $data['nickname'] . '2' : $data['nickname']) : '';
        ...
        if ($invite && ($member = $this->get_base_member($invite))) {
            $this->db->insert('member_invite', array(
                'uid' => $invite,
                'rid' => $uid,
            ...
            if ($value['invite_experience']) {
                $this->update_score(0, $invite, (int)$value['invite_experience'], 'invite-'.$uid, "lang,m-211,".$data['username']);
            }
            if ($value['invite_score']) {
                $this->update_score(1, $invite, (int)$value['invite_score'], 'invite-'.$uid, "lang,m-211,".$data['username']);
            }
            unset($value, $member);
        }
    ...
    public function member_uid($login = 0) {
        ...
        if (!$login && IS_MEMBER
            && $uid = $this->session->userdata('member_auth_uid')) {
            return $uid;
        } else {
            ...
            if (substr(md5(SYS_KEY . $uid), 5, 20) !== $cookie) {
                return NULL;
            }
    ...
    public function setting($isdomain = FALSE) {
        ...
        foreach ($setting as $t) {
            ...
            if (strpos($t['name'], '_') !== FALSE
                && strpos($t['name'], 'sns_') === FALSE) {
                $data['_'.$t['name']] = $t['value'];
            } elseif ($t['name'] == 'permission'
                || $t['name'] == 'pay'
                || $t['name'] == 'space'
                || $t['name'] == 'domain') {
            ...
            // 返回域名信息
            if ($isdomain && $t['name'] == 'domain' && $data['domain']) {
    ...
    public function cache() {
        ...
        foreach ($this->SITE as $sid => $t) {
            ...
            if ($t['SITE_MOBILE']) {
                // 移动端域名
                $cache['synurl'][] = 'http://'.$t['SITE_MOBILE'].'/member';
            }
            // 将站点的域名配置文件加入同步列表中
    ...
    private function _where(&$select, $param) {
        ...
        if ($param['search'] == 1) {
            ...
            if (isset($data['keyword']) && $data['keyword'] && $data['field']) {
                ...
                } elseif (in_array($data['field'], array('phone', 'name', 'email'))) {
    ...
    public function limit_page($param, $page, $total) {
        ...
        list($_param, $_search) = $this->_where($select, $param);
        $order = isset($_GET['order']) && strpos($_GET['order'], "undefined") !== 0 ? $this->input->get('order') : 'uid desc';
    ...
    public function update_score($type, $uid, $val, $mark, $note = '', $count = 0) {
        ...
        if ($type) {
            $this->db // 更新虚拟币
                 ->where('uid', (int) $uid)
                 ->update('member', array('score' => $value));
        } else {
            $this->db // 更新积分
                 ->where('uid', (int) $uid)
                 ->update('member', array('experience' => $value));
        }
        unset($value);
    ...
    public function delete($uids) {
        ...
        foreach ($uids as $uid) {
            ...
            // 删除session值
            $this->db->where('user_id', $uid)->delete('member_session');
        }
        // 删除空间
