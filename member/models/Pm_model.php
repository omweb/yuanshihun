...
class Pm_model extends CI_Model{
	...
	public function limit_page($uid, $page) {
		...
		$data = $this->db
					 ->query("SELECT * FROM ".$this->db->dbprefix('pm_members')." m LEFT JOIN ".$this->db->dbprefix('pm_lists')." t ON t.plid=m.plid WHERE m.uid=$uid ORDER BY m.lastdateline DESC LIMIT ".$this->pagesize * ($page - 1).", ".$this->pagesize)
					 ->result_array();
	...
	public function read_limit_page($touid, $page) {
		...
		$sql = "SELECT t.*, p.*, t.authorid as founderuid, t.dateline as founddateline FROM ".$this->db->dbprefix($this->_pm_tablename($plid))." p LEFT JOIN ".$this->db->dbprefix('pm_lists')." t ON p.plid=t.plid WHERE $addsql ORDER BY p.dateline DESC LIMIT ".$this->pagesize * ($page - 1).", ".$this->pagesize;
