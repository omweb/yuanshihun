...
class Api extends M_Controller {
			...
			foreach ($data as $t) {
				if (is_dir($path.'/'.$t)) {
					...
					$list[] = array(
						'type' => 'dir',
						'name' => $name,
						'icon' => SITE_URL.'omooo/statics/images/ext/dir.gif',
					...
					if ($ext != 'php' && in_array($ext, $exts)) {
						$list[] = array(
							...
							'icon' => is_file(FCPATH.'omooo/statics/images/ext/'.$ext.'.gif') ? SITE_URL.'omooo/statics/images/ext/'.$ext.'.gif' : SITE_URL.'omooo/statics/images/ext/blank.gif',
						);
					}
				}
			}
	...
	public function thumb2() {
        ...
        // 图片不存在时调用默认图片
        if (!is_file($file)) {
            $file = FCPATH.'omooo/statics/images/nopic.gif';
        }
	...
	public function thumb() {
		...
		$file = $info && in_array($info['fileext'], array('jpg', 'gif', 'png')) ? $info['attachment'] : 'omooo/statics/images/nopic.gif'; // 图片判断
