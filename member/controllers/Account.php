...
class Account extends M_Controller {
	...
	public function permission() {
		...
		if ($page == 0) {
			...
			$content.= '  <tr><td align="right">'.lang('m-275').'：&nbsp;</td><td>&nbsp;<img src="'.SITE_URL.'omooo/statics/images/'.(int)$rule['is_upload'].'.gif"></td>';
			$content.= '  <td align="right">'.lang('m-276').'：&nbsp;</td><td>&nbsp;<img src="'.SITE_URL.'omooo/statics/images/'.(int)$rule['is_download'].'.gif"></td></tr>';
			...
			<tr>
				...
				<td><img src="'.SITE_URL.'omooo/statics/images/'.(int)$setting['verifyspace'].'.gif"></td>
				<td width="120" align="right">'.lang('m-144').'：&nbsp;</td>
				<td><img src="'.SITE_URL.'omooo/statics/images/'.(int)$this->get_cache('member', 'group', $this->member['groupid'], 'allowspace').'.gif"></td>
			</tr>
			</table>
			<table class="dr_table" width="100%" border="0">
			<tr>
			...
			foreach ($model as $t) {
				...
				$content.= '<td align="center"><img src="'.SITE_URL.'omooo/statics/images/'.(int)$rule['use'].'.gif"></td>';
				$content.= '<td align="center"><img src="'.SITE_URL.'omooo/statics/images/'.($rule['verify'] ? 1 : 0).'.gif"></td>';
				...
				if ($key == $page) {
					...
					foreach ($cat as $c) {
						if (!$c['child']) {
							...
							$content.= '<td align="center"><img src="'.SITE_URL.'omooo/statics/images/'.($rule['show'] ? 0 : 1).'.gif"></td>';
							$content.= '<td align="center"><img src="'.SITE_URL.'omooo/statics/images/'.(int)$rule['add'].'.gif"></td>';
							$content.= '<td align="center"><img src="'.SITE_URL.'omooo/statics/images/'.(int)$rule['edit'].'.gif"></td>';
							$content.= '<td align="center"><img src="'.SITE_URL.'omooo/statics/images/'.(int)$rule['del'].'.gif"></td>';
							$content.= '<td align="center"><img src="'.SITE_URL.'omooo/statics/images/'.($rule['verify'] ? 1 : 0).'.gif"></td>';
							$content.= '<td align="center">'.($rule['postnum'] ? $rule['postnum'] : lang('m-278')).'</td>';
							$content.= '<td align="center">'.($rule['postcount'] ? $rule['postcount'] : lang('m-278')).'</td>';
