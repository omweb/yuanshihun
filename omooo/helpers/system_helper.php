...
function dr_file_info($key) {
	...
	if (is_numeric($key)) {
		...
		if (in_array($info['fileext'], array('jpg', 'gif', 'png'))) {
			$info['icon'] = SITE_URL.'omooo/statics/images/ext/jpg.gif';
		} else {
			$info['icon'] = is_file(FCPATH.'omooo/statics/images/ext/'.$info['fileext'].'.gif') ? SITE_URL.'omooo/statics/images/ext/'.$info['fileext'].'.gif' : SITE_URL.'omooo/statics/images/ext/blank.gif';
		}
		$info['size'] = dr_format_file_size($info['filesize']);
		return $info;
	} else {
		return array('icon' => SITE_URL.'omooo/statics/images/ext/url.gif', 'size' => '');
	}
}
