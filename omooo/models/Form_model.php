...
class Form_model extends CI_Model {
	...
	public function add($data) {
		...
		if ($this->link->insert($this->prefix, $data)) {
			...
			.'require FCPATH.\'omooo/core/D_Form.php\';'.PHP_EOL.PHP_EOL
	...
	public function del($id, $sid = SITE_ID) {
		...
		$name = 'Form_'.$sid.'_'.$id;
		@unlink(FCPATH.'omooo/controllers/'.$name.'.php');
		@unlink(FCPATH.'omooo/controllers/admin/'.$name.'.php');
		@unlink(FCPATH.'omooo/templates/admin/form_'.$id.'.html');
	...
	public function limit_page($table, $param, $page, $total) {
		...
		if (!$total || $param['search']) {
			...
			if ($param['keyword']) {
                $this->link->like('title', $param['keyword']);
            }
