...
class Module_model extends CI_Model {
	...
	public function add($dir, $config, $mydb = '') {
        ...
        if (!$mydb) {
            ...
            if ($extend) {
                // 字段入库
                $main = require FCPATH.$dir.'/config/extend.main.table.php'; // 主表信息
                foreach ($main['field'] as $field) {
                    $this->add_field($id, $field, 1, 1);
                }
                $data = require FCPATH.$dir.'/config/extend.data.table.php'; // 附表信息
                if ($data['field']) {
                    foreach ($data['field'] as $field) {
	...
	public function export($dir, $name) {
		...
		if ($config['extend']) {
			// 内容扩展表字段
			$file = FCPATH.$dir.'/config/extend.main.table.php';
			$table = array();
			$header = $this->dconfig
						   ->file($file)
						   ->note('内容扩展表结构（由开发者定义）')
            ...
            // 内容扩展附表字段
            $file = FCPATH.$dir.'/config/extend.data.table.php';
            $table = array();
            $header = $this->dconfig
                           ->file($file)
                           ->note('内容扩展附表结构（由开发者定义）')
                           ->to_header();
		...
		if ($form) {
			...
			foreach ($form as $t) {
				...
				$field = $this->db
							  ...
							  ->where('relatedname', 'mform-'.$dir.'-'.SITE_ID)
							  ->order_by('displayorder ASC, id ASC')
							  ->get('field')
							  ->result_array();
				$fdata[$t['id']] = array(
	...
	public function import_form($dir) {
        ...
        $form = $db->get(SITE_ID.'_'.$dir.'_form')->result_array();
        if ($form) {
            foreach ($form as $t) {
                $this->mform_model->del($t['id']);
            }
        }
		...
		foreach ($data as $id => $form) {
			...
			foreach ($form['field'] as $t) {
				unset($t['id']);
				$t['relatedname'] = 'mform-'.$dir.'-'.SITE_ID;
			...
			if (!file_exists($file)) {
				...
				.'require FCPATH.\'omooo/core/D_Admin_Form.php\';'.PHP_EOL.PHP_EOL
			...
			if (!file_exists($file)) {
				file_put_contents($file, '<?php'.PHP_EOL.PHP_EOL
				.'require FCPATH.\'omooo/core/D_Member_Form.php\';'.PHP_EOL.PHP_EOL
			...
			if (!file_exists($file)) {
				file_put_contents($file, '<?php'.PHP_EOL.PHP_EOL
				.'require FCPATH.\'omooo/core/D_Home_Form.php\';'.PHP_EOL.PHP_EOL
     ...
     * @param	intval	$mydb       是否自定义数据模块
     * @param	intval	$_siteid    已经安装过的站点id
	 * @return	void
	 */
	public function install($id, $dir, $siteid, $config, $mydb = 0, $_siteid = 0) {
	
		if (!$id || !$dir || !$siteid
            || !isset($this->site[$siteid])) {
            return NULL;
        }
        ...
        if (!$mydb) {
            ...
            if (!$sql) {
                // 从本地配置中获取表结构
                $cfg = require FCPATH.$dir.'/config/main.table.php';
                $sql = $cfg['sql'];
            }
            ...
            if (!$sql) {
                // 从本地配置中获取表结构
                $cfg = require FCPATH.$dir.'/config/data.table.php';
                $sql = $cfg['sql'];
            }
            ...
            if ($extend) {
                ...
                if (!$sql) {
                    // 从本地配置中获取表结构
                    $cfg = require FCPATH.$dir.'/config/extend.main.table.php';
                    $sql = $cfg['sql'];
                }
                ...
                if (!$sql) {
                    // 从本地配置中获取表结构
                    $cfg = require FCPATH.$dir.'/config/extend.data.table.php';
                    $sql = $cfg['sql'];
                }
		...
		// 插入初始化数据
		if (is_file(FCPATH.$dir.'/config/install.sql')
            && $install = file_get_contents(FCPATH.$dir.'/config/install.sql')) {
			$_sql = str_replace(
				array('{tablename}', '{dbprefix}', '{moduleid}', '{moduledir}', '{siteid}'), 
				array($prefix, $this->db->dbprefix, $id, $dir, SITE_ID), 
				$install
			);
			$sql_data = explode(';SQL_OmWeb_EOL', trim(str_replace(array(PHP_EOL, chr(13), chr(10)), 'SQL_OmWeb_EOL', $_sql)));
	...
	public function uninstall($id, $dir, $siteid, $delete = 0) {
        ...
        $mydb = isset($config['mydb']) && $config['mydb'] ? 1 : 0;
	...
	public function edit($_data, $data) {
        ...
        $mcfg = require FCPATH.$_data['dirname'].'/config/module.php';
        $mydb = isset($mcfg['mydb']) && $mcfg['mydb'] ? 1 : 0;
        $_siteid = 0; // 已有站点id，用于增加站点时使用
        ...
        foreach ($_data['site'] as $siteid => $cfg) {
            ...
            if (!isset($data['site'][$siteid]['use']) ||
                !$data['site'][$siteid]['use']) {
		...
		foreach ($data['site'] as $siteid => $cfg) {
			if (!isset($_data['site'][$siteid]['use']) ||
                !$_data['site'][$siteid]['use']) {
				$this->install($_data['id'], $_data['dirname'], $siteid, $mcfg, $mydb, $_siteid);
			}
		}
                    ...
                    if ($form) {
                        foreach ($form as $t) {
                            ...
                            $field = $this->db
                                          ->where('disabled', 0)
                                          ->where('relatedid', $t['id'])
                                          ->where('relatedname', 'mform-'.$data['dirname'].'-'.$siteid)
                                          ->order_by('displayorder ASC, id ASC')
                                          ->get('field')
                                          ->result_array();
                            if ($field) {
