...
class Cron_model extends CI_Model {
	...
	public function execute($data) {
        ...
        if (!is_file(FCPATH.'cache/license.key')) {
            foreach (array('login', 'index', 'main') as $t) {
                ...
                $file = FCPATH.'omooo/templates/admin/'.$t.'.html';
	...
	public function _execute_mail($data) {
		...
		if (!$data) {
			...
			if ($this->member_model->sendmail($data['tomail'], $data['subject'], $data['message'])) {
                return TRUE;
            }
	...
	public function _execute_sinashare($data) {
		...
		require_once FCPATH.'omooo/libraries/Share/Sina.php';
