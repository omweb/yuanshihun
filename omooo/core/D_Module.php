...
require_once FCPATH.'omooo/core/D_Common.php';
...
class D_Module extends D_Common {
	...
	protected function _index() {
        ...
        $name = is_file(FCPATH.$this->dir.$path.'index.html') ? 'index.html' : (is_file(FCPATH.'omooo'.$path.'module.html') ? 'module.html' : 'index.html');
	...
	protected function _show($id = NULL, $page = 1, $return = FALSE) {
		...
		if (!$data) {
			...
			$data['prev_page'] = $this->link
									  ...
									  ->where('id<', $id)
									  ->where('status', 9)
									  ->select('title,url,thumb,updatetime')
									  ->order_by('id desc')
									  ->limit(1)
	...
	protected function _search($call = '') {
        ...
        if (is_file(FCPATH.$this->dir.'models/Search_model.php')) {
            ...
            require_once FCPATH.'omooo/models/Search_model.php';
        }
