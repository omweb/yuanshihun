...
class D_File extends M_Controller {
	...
	public function index() {
		...
		if ($data) {
			foreach ($data as $t) {
				$ext = strrchr($t, '.');
				if ($ext && !in_array($ext, array('.html', '.js', '.css'))) {
                    continue;
                }
	...
	public function add() {
		...
		$error = $file = '';
		if (!is_dir($path)) {
            exit('<p style="padding:10px 20px 20px 20px">'.lang('221').'</p>');
        }
    ...
    public function store() {
        $this->template->assign(array(
            'url' => 'http://store.omooo.com/index.php?c=category&id=2&action=application&param='.dr_base64_encode(dr_array2string(array(
                'site' => SITE_URL,
                'name' => SITE_NAME,
                'admin' => SELF,
