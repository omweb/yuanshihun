...
class D_Member_Form extends M_Controller {
    ...
    public function __construct() {
		...
		if (!$this->form) {
            $this->member_msg(lang('mod-108'));
        }
		if ($this->form['permission'][$this->markrule]['disabled']) {
            $this->member_msg(lang('mod-101'));
        }
    ...
    public function index() {
		...
		$tpl = is_file(APPPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_data_'.SITE_ID.'_'.$this->form['id'].'.html') ? 'mform_data_'.SITE_ID.'_'.$this->form['id'].'.html' : 'mform_data.html';
		...
		if ($this->input->get('action') == 'search') {
			...
			// 首次请求
			$url = 'index.php?s='.APP_DIR.'&c='.$this->router->class.'&m=index&action=search&kw='.$kw.'&order='.$order.'&cid='.$cid;
			$this->template->assign(array(
			...
			$this->template->display(is_file(APPPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_index_'.SITE_ID.'_'.$this->form['id'].'.html') ? 'mform_index_'.SITE_ID.'_'.$this->form['id'].'.html' : 'mform_index.html');
		}
    }
    ...
    public function listc() {
		...
		if ($this->input->get('action') == 'search') {
			...
			$this->template->display(is_file(APPPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_cdata_'.SITE_ID.'_'.$this->form['id'].'.html') ? 'mform_cdata_'.SITE_ID.'_'.$this->form['id'].'.html' : 'mform_cdata.html');
		} else {
			// 首次请求
			$this->template->assign(array(
				'list' => $this->link->limit($this->pagesize)->get($this->table)->result_array(),
				...
				'searchurl' => 'index.php?s='.APP_DIR.'&c='.$this->router->class.'&m=listc&action=search&cid='.$cid.'&kw='.$kw.'&order='.$order,
				'meta_name' => $cdata['title'],
			));
			$this->template->display(is_file(APPPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_listc_'.SITE_ID.'_'.$this->form['id'].'.html') ? 'mform_index_'.SITE_ID.'_'.$this->form['id'].'.html' : 'mform_listc.html');
		}
    }
    ...
    public function edit() {
		...
		$tpl = APPPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_edit_'.SITE_ID.'_'.$this->form['id'].'.html';
		$this->template->assign(array(
			'data' => $data,
			'error' => $error,
    ...
    public function show() {
		...
		$tpl = APPPATH.'templates/member/'.MEMBER_TEMPLATE.'/mform_show_'.SITE_ID.'_'.$this->form['id'].'.html';
		$this->template->assign(array(
