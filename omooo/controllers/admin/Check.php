 ...
 *
 * @since		version 2.3.0
 * @author		Chunjie <chunjie@omooo.com>
...
class Check extends M_Controller {
    ...
    private function _upload() {
        ...
        $str = '';
        if ($file >= $post) {
            $str.= $this->halt('系统配置不合理，post_max_size值必须大于upload_max_filesize值，否则会出现“进度条100%卡住”或者提示“游客不允许上传”，解决方案：<a style="color:red" target="_blank" href="http://www.OmWeb.net/forum.php?mod=viewthread&tid=2490">这里</a>', 0);
        }
        if ($file < 10) {
            $str.= $this->halt('系统环境只允许上传'.$file.'MB文件，可以设置upload_max_filesize值提升上传大小，解决方案：<a style="color:red" target="_blank" href="http://www.OmWeb.net/forum.php?mod=viewthread&tid=2490">这里</a>', 1);
        }
        if ($post < 10) {
            $str.= $this->halt('系统环境要求每次发布内容不能超过'.$post.'MB（含文件），可以设置post_max_size值提升发布大小，解决方案：<a style="color:red" target="_blank" href="http://www.OmWeb.net/forum.php?mod=viewthread&tid=2490">这里</a>', 1);
        }
	...
	private function halt($msg, $status = 1) {
		...
		return $status ? "<tr><td align=\"left\"><font color=green><img width=\"16\" src=\"".SITE_URL."omooo/statics/images/ok.png\">&nbsp;&nbsp;".$msg."</font></td></tr>" : "<tr><td align=\"left\"><font color=red><img width=\"16\" src=\"".SITE_URL."omooo/statics/images/b_drop.png\">&nbsp;&nbsp;".$msg."</font></td></tr>";
	}
}