...
class Route extends M_Controller {
    ...
    public function index() {
		...
		if (strpos($server, 'apache') !== FALSE) {
			...
			.'        <rewrite>'.PHP_EOL
			.'            <rules>'.PHP_EOL
			.'		<rule name="OmWeb" stopProcessing="true">'.PHP_EOL
			.'		    <match url="^(.*)$" />'.PHP_EOL
			...
			$name = $server;
			$note = '建议使用isapi_rewrite第三版,老版本的rewrite不支持RewriteCond语法（<a style="color:blue" href="http://www.OmWeb.net/forum.php?mod=viewthread&tid=1823&extra=" target="_blank">老版本httpd.ini请看这里</a>）<br><font color=red><b>将以下内容保存为.htaccess文件，放到网站根目录</b></font>';
			$code = 'RewriteEngine On'.PHP_EOL
			.'RewriteBase /'.PHP_EOL
			.'RewriteCond %{REQUEST_FILENAME} !-f'.PHP_EOL
			.'RewriteCond %{REQUEST_FILENAME} !-d'.PHP_EOL
