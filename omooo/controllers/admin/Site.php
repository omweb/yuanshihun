...
class Site extends M_Controller {
    ...
    public function config() {
		...
		$files = directory_map(FCPATH.'omooo/statics/watermark/', 1);
		$opacity = array();
		foreach ($files as $t) {
			if (substr($t, -3) == 'ttf') {
				$font[] = $t;
		...
		$this->template->assign(array(
			...
			'lang' => dr_dir_map(FCPATH.'omooo/language/', 1),
			'theme' => array_diff(dr_dir_map(FCPATH.'omooo/statics/', 1), array('css', 'images', 'watermark')),
			'result' => $result,
			'navigator' => @explode(',', $data['SITE_NAVIGATOR']),
			'wm_opacity' => $opacity,
            'remote_type' => array(0 => lang('close'), 1 => 'FTP', 2 => '百度云存储BCS', 3 => '阿里云存储OSS'),
			'wm_font_path' => $font,
			'template_path' => array_diff(dr_dir_map(FCPATH.'omooo/templates/', 1), array('api', 'admin')),
