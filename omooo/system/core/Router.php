...
class CI_Router {
    ...
    protected function _set_routing()
    {
        ...
        if ($this->enable_query_strings)
        {
            ...
            else
            {
                ...
                if (file_exists(APPPATH.'config/routes.php')) {
                    ...
                    if (file_exists(APPPATH.'config/rewrite.php')) {
                        include(APPPATH.'config/rewrite.php');
                    }
                } else {
                    include(FCPATH.'config/routes.php');
                    if (file_exists(FCPATH.'config/rewrite.php')) {
                        include(FCPATH.'config/rewrite.php');
                    }
                }

                $routes = (empty($route) OR ! is_array($route)) ? array() : $route;
                unset($route);
                ...
                if (defined('DR_URI')) {
                    ...
                    if (!$uri) {
                        if (!is_file(FCPATH.'cache/install.lock') && is_dir($_SERVER['DOCUMENT_ROOT'].'/'.DR_URI)) {
                            ...
                            show_error('OmWeb禁止子目录安装，请放置在网站根目录安装', 404);
                        } else {
                            $this->set_class('api');
        ...
        if (file_exists(ENVIRONMENT.'/routes.php'))
        {
