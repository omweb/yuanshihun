...
class Dmail {
    ...
    public function send($toemail, $subject, $message) {
	    ...
	    //$email_from = $from == '' ? '=?'.$cfg['charset'].'?B?'.base64_encode($cfg['auth_username'])."?= <".$cfg['from'].">" : (preg_match('/^(.+?) \<(.+?)\>$/', SITE_NAME, $mats) ? '=?'.$cfg['charset'].'?B?'.base64_encode($mats[1])."?= <$mats[2]>" : $from);
	    $email_from = '=?'.$cfg['charset'].'?B?'.base64_encode(SITE_NAME)."?= <".$cfg['from'].">";
