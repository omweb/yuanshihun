...
class F_Ueditor extends A_Field {
            ...
            foreach ($imgs[3] as $i => $img) {
                ...
                if ($down && $uid) {
                    ...
                    if (is_array($result)) {
                        ...
                        $timg = 0;
                        $value = str_replace($imgs[0][$i], " id=\"OmWeb_img_$id\" src=\"".dr_file($file)."\"", $value);
                        $attach[] = $id;
                    }
                }
                // 当不下载图片
                if ($timg) {
                    if (preg_match('/<img id="OmWeb_img_([0-9]+)"([\w|\s|=\."]*)src="'.str_replace('/', '\/', $img).'"/iU', $value, $match)) {
                        $attach[] = (int)$match[1];
                    } elseif (preg_match('/<img src="'.str_replace('/', '\/', $img).'"(.*)id="OmWeb_img_([0-9]+)"/iU', $value, $match)) {
                        $attach[] = (int)$match[2];
                    } else {
    ...
    public function get_attach_id($value) {
        ...
        $data = array();
        $pregs = array('/id="OmWeb_img_([0-9]+)"/iU');
