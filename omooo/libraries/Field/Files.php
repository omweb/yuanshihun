...
class F_Files extends A_Field {
	...
	public function input($cname, $name, $cfg, $value = NULL, $id = 0) {
			...
			foreach ($value['file'] as $id => $fileid) {
				...
				<tr>
					<td width="80" style="text-align:right">
						'.($id+1).'、
						<a href="javascript:;" title="'.lang('edit').'" onclick="dr_edit_file(\''.$url.'&count=1\',\''.$name.'\',\'999'.$id.'\')"><img align="absmiddle" src="'.SITE_URL.'omooo/statics/images/b_edit.png"></a>
						<a href="javascript:;" title="'.lang('del').'" onclick="dr_remove_file(\''.$name.'\',\'999'.$id.'\')"><img align=\"absmiddle\" src="'.SITE_URL.'omooo/statics/images/b_drop.png"></a>
					</td>
					<td>
		...
		if (!defined('OmWeb_FILES_LD')) {
			...
			define('OmWeb_FILES_LD', 1);//防止重复加载JS
		}
